...
object scalatagsPrinter extends Xml2ScalatagMaps{
  def print(stag: STag, indentLevel: Int = 0): String = stag match {
    case htag: HtmlTag  => {
      ...
      } else if(tagMap.contains(htag.tag)) "()" else ""
      ...
      val otherNamedAttributes = for((k,v) <- htag.attrMap if attrMap.contains(k)) yield {
        '.' + k + "(\"" + slashEscaper.escape(v.toString) + "\")"
      }
      val otherUnnamedAttributes = htag.attrMap.filterKeys(!attrMap.contains(_)).map{ case (k,v) => slashEscaper.escape(k) + "\" -> \"" + slashEscaper.escape(v.toString) }
